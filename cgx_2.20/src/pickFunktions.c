...
#define MAX_BUF      100
#define CUTELEMTOL   0.01         // fraction of length between cornernodes used to judge if a certain element is cutted (qcut)
...
typedef struct {
  ...
  int nnew[CUTS];
  int nnew2[CUTS];
  int nref[CUTS];        // stores the referenced element node if the new node is 'close' 
} CutParam;
  ...
  else if (flag==1)
  {
    ...
    if (p1!=p2)
    {
      ...
      else if (pm>-1)
      {
        ...
        if(v_betrag(nm12)==0.) return(OUTSIDE);
...
void cutElement(int elnr, int nset, int eset, double *en, double *pn, double *eu, double *ev, CutParam *cutParam)
{
  ...
  lengthTolMax=1.+CUTELEMTOL;
  lengthTolMin=-CUTELEMTOL;
  refTolMax=1.-CUTELEMTOL;
  refTolMin=CUTELEMTOL;
  ...
  if (e_enqire[elnr].type == 3)  /* TET4 */
  {
    for(n1=0; n1<3; n1++)
    {
      ...
      v_norm( n1n2, eg );
      g = AsplitL( n1pn, eu, ev, eg, en );
      //printf("n %d %d g:%e sprod:%e\n",e_enqire[elnr].nod[n1],e_enqire[elnr].nod[n2],g,v_sprod(n1n2,en));
      if(g==MAX_FLOAT) continue;
      ns[3] = g/v_betrag(n1n2);
      if(ns[3]>1.00000) continue;
      if(ns[3]<0.00000) continue;
      if(ns[3]>refTolMax) cutParam->nref[anz_n]=e_enqire[elnr].nod[n2];
      if(ns[3]<refTolMin) cutParam->nref[anz_n]=e_enqire[elnr].nod[n1];
      v_scal( &g, eg, n1ns );
      v_add( &node[e_enqire[elnr].nod[n1]].nx, n1ns, ns );
  ...
  if(anz_n>0)
  {
    ...
    for(j=0; j<cutParam->anz_n; j++)
    {
      ...
      //printf(" noderef:%f %f\n",cutParam->npre[cutParam->nnew[j]][4],cutParam->npre[cutParam->nnew[j]][5]);
      k++;
    }
    free(rsort);
  }
...
{
  int i,j,k,e,nn,nset,eset,anz_e,anz_nset,anz_nini,nel[3];
  static double p[3][3];
  double v02[3], v01[3], en[3], vn[3], pn[3], eu[3], ev[3], dl, max_dl=0.;
  char  addDispFlagLocal=0;
  CutParam *cutParam;
  int n1,n2,foundNode,elnod,newnodeFlag=0;
  int *elNodeRef;
  ...
  if ((qcutCounter == 3)||(flag=='v'))
  {
    ...
    for(e=0; e<anz_e; e++)
    {
	  ...
	  foundNode=0;
	  //printf(" nref %f %f\n",cutParam[i].npre[cutParam[i].nnew[j]][4],cutParam[i].npre[cutParam[i].nnew[j]][5]);
	  if(cutParam[i].npre[cutParam[i].nnew[j]][4]>cutParam[i].npre[cutParam[i].nnew[j]][5])
	  {
	    n1=cutParam[i].npre[cutParam[i].nnew[j]][4];
	    n2=cutParam[i].npre[cutParam[i].nnew[j]][5];
	  }
	  else
	  {
	    n2=cutParam[i].npre[cutParam[i].nnew[j]][4];
	    n1=cutParam[i].npre[cutParam[i].nnew[j]][5];
	  }
	  ...
	  if(!foundNode)
	  {
	    // do not create a new node if cutParam[i].nref[cutParam[i].nnew[j]] references a valid element node
	    elnod=cutParam[i].nref[cutParam[i].nnew[j]];
	  ...
	  else cutParam[i].nnew2[j]=foundNode;
        }
        /* generiere neue elemente */
        for(j=1; j<cutParam[i].anz_n-1; j++)
        {
          nel[0]=cutParam[i].nnew2[0];
          nel[1]=cutParam[i].nnew2[j];
          nel[2]=cutParam[i].nnew2[j+1];
