  ...
  if(set[setNr].anz_n>0)
  {
    ...
    if(anz->l)
    {
      ...
      if((transtyp==3)||(transtyp==5))
      {
        ...
        if(pkt1[0]=='x') { csab[3]=1.; fi=atof(pkt2)*PI/180.; }
        else if(pkt1[0]=='y') { csab[4]=1.; fi=atof(pkt2)*PI/180.; }
        else if(pkt1[0]=='z') { csab[5]=1.; fi=atof(pkt2)*PI/180.; }
        else
        {
          pnr1=getPntNr( pkt1 );
          pnr2=getPntNr( pkt2 );
