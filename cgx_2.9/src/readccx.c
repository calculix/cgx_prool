  ...
  do{
    ...
    if(face>=0)
    {
      if (el==0)
      {
        ...
        for(k=0; k<setx[eset].anz_e; k++)
        {
          i=setax(setNr,"j",0);
          if(i>-1)
  	  {
            setx[setNr].elf[i].e=setx[eset].elem[k];
            setx[setNr].elf[i].f=face;
  	  }
          for(i=dof[0]-1; i<=dof[1]-1; i++)
	  {
            ...
            if(j>-1)
            {
              setx[setNr_dof[i]].elf[j].e=setx[eset].elem[k];
              setx[setNr_dof[i]].elf[j].f=face;
            }
            writeFaceVals(anzx_l+i, setx[eset].elem[k], face, 1, &vals[i], 0);
	  }
        }
      }
      else
