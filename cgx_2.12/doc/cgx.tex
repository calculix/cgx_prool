...
\begin{document}
\title{CalculiX USER'S MANUAL\\ - CalculiX GraphiX, Version 2.12 -}
\author{Klaus Wittig}
\maketitle
\begin{figure}[h]
...
\begin{verbatim}
   'frame' 
\end{verbatim}
This command fits the model into the drawing space. 
...
\item Use the interface program to convert the cad format to fbd format (You find example data (vda) in CalculiX/cgx\_x.x/examples/cad)\begin{verbatim} (ie: vda2fbd halter.vda > halter.fbd). \end{verbatim} In some cases the header of the vda file causes trouble and must be modified to make the program run.
