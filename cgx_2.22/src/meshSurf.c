  ...
  int  sum_div;
    ...
    if (surf[j].nl!=MAX_EDGES_PER_SURF)
    {
      ...
      else if (surf[j].nl>4)
      {
        ...
        getNewName( name, "s" );
        if(surf[j].nc==0) s=surface_i( name, surf[j].ori, surf[j].sh, surf[j].nl, surf[j].o, surf[j].l, surf[j].typ );
        else s=surface_i( name, surf[j].ori, surf[j].sh, surf[j].c[0], surf[j].o, surf[j].l, surf[j].typ );
        if( s<0)
        { printf("ERROR: surface could not be created\n"); return(-2); }
        pre_seta(specialset->zap, "s", name);
        surf[s].etyp=surf[j].etyp;
...
/*
  anz_s: set[setNr].anz_s, only needed for cfd meshes
  sindx: surface_nr
  meshflag: if 1 surfaces w/o element assignment have to be meshed (for hex bodies) even if no eltype is defined
  renderFlag: no elems if 1
*/
int meshSurf( int anz_s, int sindx, int meshflag, int renderFlag)
{
  int  ii,jj,i,j,k,l,n,m, s=0, ke,u,v, b=0,a, amax,bmax;
  ...
  char surFlag;
  int transitionflag, ipuf=0, nori[2], eori[2], ebuf[26], nmax;
  Gsur surfbuf[1];
  int jbuf,Stmp;
  int returnVal=0;
  ...
  /* evaluate the surf */
	...
	if(surf[j].typ[i]=='l')
	{
	  if(line[surf[j].l[i]].div<2) { printf(" ERROR: surf:%s with 3 edges uses at least one line:%s with div <2, this is not permitted. surf not meshed.\n\n", surf[j].name, line[surf[j].l[i]].name); returnVal=-2; return(returnVal); }
	  //if((surf[j].etyp==7)&&(line[surf[j].l[i]].div<2)) { printf(" ERROR: surf:%s with 3 edges uses at least one line:%s with div <2, this is not permitted. surf not meshed.\n\n", surf[j].name, line[surf[j].l[i]].name); returnVal=-2; return(returnVal); }
	  //if((surf[j].etyp==8)&&(line[surf[j].l[i]].div<4)) { printf(" ERROR: surf:%s with 3 edges uses at least one line:%s with div <4, this is not permitted. surf not meshed.\n\n", surf[j].name, line[surf[j].l[i]].name); returnVal=-2; return(returnVal); }
	}
      }
    }
    ...
    if(surf[j].sref>-1) j=surf[j].sref;
  ...
  printf("hallo3 unbalanced:%d\n",jbuf);
    ...
    sem_post(&sem_nurs);
        ...
        if(surf[jbuf].sref>-1) s=j; j=jbuf;
        surf[j].npgn=0;
    ...
    sem_post(&sem_n);
        ...
        if(surf[jbuf].sref>-1) j=s; /* change back to substitute surf */
  ...
  printf("hallo if a substitute surf was meshed then map the mesh onto the original one:%d\n",jbuf);
    ...
    j=jbuf;
    if((surf[j].etyp==7)&&(surf[j].ne>0)&&((surf[j].sref>-1)||(transitionflag)))
    {
  ...
  {
    s=surf[jbuf].sref;
    //printf(" sref:%s nn:%d ne:%d\n", surf[s].name, surf[s].nn, surf[s].ne);
    if(surf[s].nn>0)
    {
      //crash?? free(surf[s].nod);;
      surf[s].nn=0;
    }
    if(surf[s].ne>0)
    {
      //crash?? free(surf[s].elem);;
      surf[s].ne=0;
    }
  }
  free(div_l);
