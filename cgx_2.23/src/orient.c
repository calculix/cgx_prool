...
int orientLcmb( int nr )
{
  ...
  if (i==lcmb[nr].nl)
  {
    errMsg(" ERROR: lcmb %s is a closed loop, no orientation possible\n", lcmb[nr].name);
...
#define TEST 0
  ...
  if(surf[nr].nl<1) return(-1);
  if(printFlag) printf (" orient surf:%s \n", surf[nr].name);
#if TEST    
  printf (" orient surf:%s nl:%d\n", surf[nr].name, surf[nr].nl);
    for(i=0; i<surf[nr].nl; i++)
      printf ("  surf[nr].o[i]:%c surf[nr].typ[n]:%c surf[nr].l[n]:%d\n", surf[nr].o[i],surf[nr].typ[n],surf[nr].l[n]);
  ...
  /* die einzelnen Linenzuege werden zum NURBS-Trimming benutzt. Zum strukturierten Vernetzen sind */
  ...
  do
  {
...
#if TEST    
    printf ("l:%s  p1:%s p2:%s  \n", line[surf[nr].l[i]].name, point[p1].name, point[p2].name );
#endif
      }
    ...
    if (i==surf[nr].nl)
    {
      if(surf[nr].typ[n]=='l') errMsg("WARNING: line[%d]:%s not connected\n",n,line[surf[nr].l[n]].name);
      //surf[nr].typ[n]='d';
      if(surf[nr].typ[n]=='l')  surf[nr].typ[n]='L'; else surf[nr].typ[n]='C';
      n++;
      if(n==surf[nr].nl)
      {
    ...
    closedLoopFlag=0;
    for (i=0; i<surf[nr].nl; i++) /* suche die anschliessende durch vergleich (with inner second loop!)*/
    {
      for (j=n+1; j<surf[nr].nl; j++) /* suche die anschliessende durch vergleich */
      {
      ...
      if(surf[nr].typ[n]=='l')
      {
        if((pnt==line[surf[nr].l[n]].p1)||(pnt==line[surf[nr].l[n]].p2))
        {
          ...
          surf[nr].c[surf[nr].nc]=lcount-lastlcount;
          surf[nr].nc++;
	  closedLoopFlag=1;
#if TEST    
      ...
      else if(surf[nr].typ[n]=='c')
      {
        if((pnt==lcmb[surf[nr].l[n]].p1)||(pnt==lcmb[surf[nr].l[n]].p2))
        {
          /* number of lines in each curve */
          if((surf[nr].c=(int *)realloc((int *)surf[nr].c, (surf[nr].nc+1)*sizeof(int)) )==NULL)
          { printf(" ERROR: realloc failure in orient surf:%s can not be oriented\n", surf[nr].name); exit(-1); }
	  ...
	  closedLoopFlag=1;
#if TEST    
          printf ("lcmb, closed line loop found, n:%d\n",n);
#endif
        }
      }
    ...
    {
      //if( (!surf[nr].o[n])&&(surf[nr].typ[n]!='d'))
       if( (!surf[nr].o[n]) && ((surf[nr].typ[n]!='L')&&(surf[nr].typ[n]!='C')) )
       {
        err--;
        break;
      }
...
#if TEST    
  printf("lcount:%d surf[nr].nl:%d\n",lcount,surf[nr].nl);
#endif

  /* re-define surf[nr] here and erase the unconnected lines */
  if(!lcount) goto orientSurfError;
  for (n=0; n<lcount; n++)
  {
      surf[nr].typ[n]=sortTyp[n];
      surf[nr].l[n]=sortLines[n];
      surf[nr].o[n]=sortOri[n];
  }
  surf[nr].nl=lcount;
  ...
  if(surf[nr].nc==0)
  {
    errMsg(" WARNING: surf:%s not chained and therefore not oriented.\n", surf[nr].name );
    err--;
  }
  ...
  for (i=0; i<surf[nr].nl; i++)
  {  
    if(surf[nr].typ[i]=='L') surf[nr].typ[i]='l';
    if(surf[nr].typ[i]=='C') surf[nr].typ[i]='c'; // back to original typ
...
int ori7SidedBody( int nr, int **cp )
{
  ...
  if (i>=body[nr].ns)
  { errMsg (" WARNING in orientBody, side %d from body:%s could not be located \n",
...
int ori6SidedBody( int nr, int **cp )
{
  ...
  */
  ...
  for (i=0; i<body[nr].ns; i++)
  {
    if (surf[body[nr].s[i]].nl<4) return(-1);
...
int oriTetBody( int nr, int **cp )
{
 ...
 more:;
  ...
  if(oriflag)
  {
    ...
    else { printf(" WARNING: too many loops. Some surfs might be still unoriented.\n"); goto orientTetBodyError; }
  }
  ...
  for(i=0; i<anzGeo->l; i++) free(ltos[i]);
  free(ltos);
  free(sori);
  free(max_dist);
  free(i_ind);
...
int orientBody( int nr )
{
  ...
  for(i=0; i<body[nr].ns; i++)
  {
    if( (cp[i]=(int *)calloc(surf[body[nr].s[i]].nl+4, sizeof(int) ) )==NULL)    
    { printf(" ERROR: malloc failure in orientBody()\n"); return(-1); }
  }
  ...
  for (i=0; i<body[nr].ns; i++)
  {
    ...
    else { errMsg (" WARNING in orientBody, surf.typ:%1c not known\n", surf[s].typ[j]); exit(-1);}
  ...
  // ignore the failed orientation since the body might still be tet meshable
  if(err<0) err=0;
