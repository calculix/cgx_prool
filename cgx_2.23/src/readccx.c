  ...
  do
  {
    ...
    else
    {
      ...
      for(i=0; i<args; i++)
      {
        ...
        if(nd[0])
	{
	  // can not be used when reading only sets during runtime
	  //setax( setNr, "n", nd[0]);
	  if(checkFlag)
	  {
	    if((nd[0]<anzx->nmin)||(nd[0]>anzx->nmax)||(nodex[nd[0]].indx<0)||(nd[0]!=nodex[nodex[nd[0]].indx].nr)||(nodex[nd[0]].pflag!=0))    
            {
	      printf(" ERROR in seta: node:%d does not exist and can not be placed in set %s, min:%d max:%d\n", nd[0],setx[setNr].name, anzx->nmin, anzx->nmax);
	      goto doNotStore;
	    }
	  }
          if ( (setx[setNr].node = (int *)realloc((int *)setx[setNr].node, (setx[setNr].anz_n+1) * sizeof(int))) == NULL )
...
int readccx(char *datin, Summen *apre, Sets **sptr, Nodes **nptr, Elements **eptr, Datasets **lptr, int checkflag )
{
  int i, j, jj, l, nr, length, args;
  static char *rec_str=(char *)NULL;
  char command[MAX_LINE_LENGTH];
  
  checkFlag=checkflag;
  
  if((rec_str = (char *)realloc((char *)rec_str, (MAX_LINE_LENGTH)*sizeof(char)) ) == NULL )
  { printf(" ERROR: realloc failed in readccx()\n"); return(-2); }
  rec_str[0]=0;
  ...
  do
  {
    ...
    else if (compare(dat[0], "*HEADING",7)==7)
    {
      ...
      // value_i can not be used in readccx.c (conflict with anz->sets, set)
      /*
      i=pre_value(command);
      if( (i=value_i( command, &rec_str[5])) <0)
	printf("ERROR: value could not be created\n");
      */
      goto checknextstr;
    }
