...
extern int  set_warnings, set_nomesh, set_glur, set_blr;
...
void pre_repSurf( int setNr)
{
  ...
  // code from fillBlendedSurf(), just sem* deleted.
...
int meshSet(char *setname, int blockFlag, int lonlyFlag, int projFlag, int meshoptFlag_length, int meshoptFlag_angle )
{
  ...
  delSet(specialset->warnings );
  set_warnings=pre_seta(specialset->warnings, "i", 0);
  ...
  /* delete specialset->warnings if nothing was stored */
  if((!set[set_warnings].anz_l)&&(!set[set_warnings].anz_s)&&(!set[set_warnings].anz_s)) delSet(specialset->warnings);
