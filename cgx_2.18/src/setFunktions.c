...
int splitLine(int l, int *l_nr, int ps_nr)
{
  ...
  else if (line[l].typ=='s')
  {
    ...
    //printf(" new seq: %d,%s,%s %d,%s,%s\n", setNr0,set[setNr0].name,trk[0], setNr1,set[setNr1].name,trk[1] );

    /* search the intervall in which the split point exists.
       This is the case if the distances between interval endpoints and plitpoint has a minimum.
    */
    setNr=line[l].trk;
    p0[0]=line[l].ip[0];
    p0[1]=line[l].ip[1];
    ...
    for (i=3; i<line[l].nip; i+=3)
    {
	  ...
	  //printf("l1 p[%d]:%s  sp %e %e \n",p,point[set[setNr].pnt[p]].name,sp1,sp2);
          if((sp1>=0.)&&(sp2>=0.))
	      seta( setNr0, "ps", set[setNr].pnt[p] );
          else break;
        }
      }
      else
