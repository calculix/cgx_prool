...
int splitLine(int l, int *l_nr, int ps_nr)
{
  ...
  double lbez, lbez_min=MAX_FLOAT, div;
  ...
  else if (line[l].typ=='s')
  {
    ...
    //printf("\n hallo line %s, new seq: %d,%s,%s %d,%s,%s\n", line[l].name, setNr0,set[setNr0].name,trk[0], setNr1,set[setNr1].name,trk[1] );
    ...
    */
    ...
    for (i=3; i<line[l].nip; i+=3)
    {
      ...
      sp1=v_sprod(p01,v01);
      v_result( p1, &point[ps_nr].px, p10  );
      sp2=v_sprod(p10,v10);
      if((sp1>=0.)&&(sp2>=0.)) { i_min=i; break; }
      p0[0]=p1[0];
      p0[1]=p1[1];
      p0[2]=p1[2];
    }
    nodeSpacing=v_betrag( v01 );
    */
    for (i=3; i<line[l].nip; i+=3)
    {
      lbez=0.;   
    ...
    //printf(" splitpnt:%s nodeSpacing:%f i_min:%d\n",point[ps_nr].name, nodeSpacing, i_min );
    ...
    for (i=3; i<line[l].nip; i+=3)
    {
	  ...
	  // the scalar product must be twice positive
          ...
          sp2=v_sprod(p10,v10);
          //printf("i:%d line[l].nip:%d p1 %f %f %f p2: %f %f %f\n",i,line[l].nip,p0[0],p0[1],p0[2],p1[0],p1[1],p1[2]);
	  //printf(" p01 %f %f %f p10: %f %f %f     sp %e %e \n",p01[0],p01[1],p01[2],p10[0],p10[1],p10[2],sp1,sp2);
	  //printf("l1 pp:%d p[%d]:%s  sp %e %e \n",pp, p,point[set[setNr].pnt[p]].name,sp1,sp2);
          if((sp1>=0.)&&(sp2>=0.))
	  {
    	    if(i==i_min)
	    {
...
#define PROJ_LOOPS 40
  ...
  if( (!functionFlag)&&( (set[setNr].anz_p==1)||(set[setNr].anz_n==1)) && (action[0]==0) )
  {
    ...
    if((set[trgtNr].anz_p==1)||(set[trgtNr].anz_n==1))
    {
      ...
      sprintf(parameter[1],"%f",distx);
      sprintf(parameter[2],"%f",disty);
      sprintf(parameter[3],"%f",distz);
      sprintf(parameter[4],"%f",da*180./PI);
      sprintf(parameter[5],"%f",dax*180./PI);
      sprintf(parameter[6],"%f",day*180./PI);
      sprintf(parameter[7],"%f",daz*180./PI);
      sprintf(parameter[8],"%f",rp);
      sprintf(parameter[9],"%f",pr[0]);
      sprintf(parameter[10],"%f",pr[1]);
      sprintf(parameter[11],"%f",pr[2]);
...
int commandoInterpreter( char *type, char *string, int na, int nb, FILE *handle1, int addFlag, int *gtolFlag )
{
  ...
  DIR *dirp;
  struct dirent *dp;
  static int ifFlag=1, whileFlag=1;
  int   returnFlag=1;
  Scale scaleSet[1];
...
// end prool commands
  ...
  else if (compareStrings(type, "SEQL")>0)
  {
    ...
    if(k==0)
    {
      for(i=0; i<set[setNr].anz_l; i++) line[set[setNr].line[i]].typ=' ';
      return(0);
    }
    l=1;
    if(set[setNr].flag=='c') seto(set[setNr].name);
  ...
  else if (compareStrings(type, "TEST")>0)
  {
	...
	strcpy(parameter[0],"TRUE");
        ...
        sprintf(parameter[2],"%f",cof[0]);
        sprintf(parameter[3],"%f",cof[1]);
        sprintf(parameter[4],"%f",cof[2]);
        sprintf(parameter[5],"%f",cof[3]);
