    ...
    if(toupper(dat[1][0])=='S')
    {
      ...
      if(face>=0)
      {
        if (el==0)
        {
          ...
          if(eset<0)
          {
            printf("ERROR: In *DFLUX set:%s not defined\n", dat[0]);
            return(-1);
          }
          for(j=0; j<setx[eset].anz_e; j++)
          {
    ...
    if(toupper(dat[1][0])=='S')
    {
      ...
      if(face>=0)
      {
        if (el==0)
        {
          ...
          if(eset<0)
          {
            printf("ERROR: In *MASSFLOW set:%s not defined\n", dat[0]);
            return(-1);
          }
          for(j=0; j<setx[eset].anz_e; j++)
          {
...
int readccx(char *datin, Summen *apre, Sets **sptr, Nodes **nptr, Elements **eptr, Datasets **lptr )
{
  ...
  do
  {
    ...
    /* get the massflow */
    else if ((compare(dat[0], "*MASSFLOW",5)==5)||(compare(dat[0], "*MASS FLOW",5)==5)) { length=getMassflow(rec_str); goto checknextstr; }
