...
void pick( unsigned char gkey, int x, int y )
{
  ...
  else if (compare(pickfunc, "qmsh", 4) == 4)
  {
    ...
    {
      if((keybuf!='s')&&(keybuf!='e')) { printf(" WARNING: The last selected entity must be the surf to split\n"); return; }
      /* delete the mesh */
...
int createElem(int n)
{
  ...
  else if(n==-1)
  {
    ...
    if     (nr==3) elem_define( anz->enext++, 7, nodes, 1, 0 );
    else if(nr==4) elem_define( anz->enext++, 9, nodes, 1, 0 );
    else if(nr==6) elem_define( anz->enext++, 8, nodes, 1, 0 );
    else if(nr==8) elem_define( anz->enext++, 10, nodes, 1, 0 );
    else
    {
  ...
  else if(n==-2)
  {
    ...
    if     (nr==8)  elem_define( anz->enext++, 1, nodes, 1, 0 );
    else if(nr==20) elem_define( anz->enext++, 4, nodes, 1, 0 );
    else
    {
      printf(" Warning, wrong number of nodes:%d, no element created, start again\n",nr);
      nr=0;
...
int splitSurf( int lmaster, int smaster )
{
  ...
  if(surf[smaster].sh>-1)
  {
    ...
    else
    {
      ...
      if(s>-1)
      {
        projSetToNurbs( nurbs, s, set, line[lmaster].trk, point);
        delNurs( 1, &s );
      }
    }
    repLine(lmaster);
...
{
  int i,j,k,n1,n2,nnew,nn1,nn2, nset,eset, nbuf;
  static double p[3][3];
        ...
        if(anz_n>0)
        {
          ...
          for(j=0; j<anz_n; j++)
          {
            if(nvalid[j])
            {
              ...
              nnew=nod( anz, &node, 0, anz->nmax+1, npre[j][0], npre[j][1], npre[j][2], 0 );
              seta(setall, "n", node[nnew].nr);
              seta(nset, "n", node[nnew].nr);
              ...
              qcut_nod[set[nset].anz_n-1].nr=node[nnew].nr;
          ...
          for(j=nbuf+1; j<set[nset].anz_n-1; j++)
          {
            ...
            elem_define( anz->enext++, 7, nvalid, 0, 0 );
