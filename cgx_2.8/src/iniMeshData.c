...
{
  int i,j,nset,apre_sets=0, ibuf=0;
  ...
  { 
    ...
    ibuf=anz->sets;
    anz->sets=0;
    if ( (readccx( datin, anz, &spre, &node, &elem, &lcase)) == -1) { exit(-1);}
    /* copy sets to filter out undefined entities */
    apre_sets=anz->sets;
    anz->sets=ibuf;
  }
  ...
  { 
    ...
    if ( (  readFoam( datin, anz, &node, &elem, &lcase)) == -1) { exit(-1);}
  }
  ...
  { 
    ...
    if ( (  readNG( datin, anz, &node, &elem, &lcase)) == -1) { exit(-1);}
  }
  ...
  if((compare(format,"ccx",3)==3)&&(apre_sets))
  {
    for(i=0; i<apre_sets; i++)
    {
      //printf("set:%s %d %d %d\n",spre[i].name, spre[i].anz_n, spre[i].anz_e, spre[i].anz_elf);
      if(spre[i].name==(char *)NULL) continue;
      ...
      for (j=0; j<spre[i].anz_elf; j++)
      {
        ...
        if((e_enqire[spre[i].elf[j].e].type>6)&&(e_enqire[spre[i].elf[j].e].type<11))
        {
          if(e_enqire[spre[i].elf[j].e].attr>3)
          {
            spre[i].elf[j].f++;
            if(e_enqire[spre[i].elf[j].e].type<9)
	    {
        ...
        // printf(" 2 e:%d f:%d\n", spre[i].elf[j].e, spre[i].elf[j].f);

        /* store the actual face-indexes in the referenced sets */
        if(face[spre[i].elf[j].e].indx[spre[i].elf[j].f]==-1)
	{
          printf(" ERROR in set:%s, face:%d of element:%d does not exist.\n",spre[i].name, spre[i].elf[j].f+1, spre[i].elf[j].e); 
	}
        else seta( nset, "f", face[spre[i].elf[j].e].indx[spre[i].elf[j].f]);
      }
    }
    for(i=0; i<apre_sets; i++) delSetx(spre[i].name);
    free(spre);
  }
  else if(compare(format,"foam",4)==4)
  { 
        ...
        if(face[set[i].elf[j].e].indx[set[i].elf[j].f]==-1)
	{
        ...
        else seta( i, "f", face[set[i].elf[j].e].indx[set[i].elf[j].f]);
      }  
    }  
  }
  else if((compare(format,"ng",2)==2)||(compare(format,"nas",3)==3))
  {
    for (i=0; i<anz->e; i++)
    {
      ...
      sprintf(buffer, "+set%d", e_enqire[e_enqire[i].nr].mat );
