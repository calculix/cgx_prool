...
int writefbd(char *setname, Summen *anz, SumGeo *anzGeo, Points *pnt, Lines *line, Lcmb *lcmb, Gsur *surf, Gbod *body, Nurbs *nurbs, Sets *set, Shapes *shape, Values *value )
{
  ...
  if( compare( setname, "all", 3)==3)
  {
    for (i=0; i<anz->sets; i++)
    {
      if(set[i].name!=(char *)NULL)
      {
        ...
        if(set[i].etyp==1) fprintf (handle1, " ELTY %s HE8%c", set[i].name, attr);
        if(set[i].etyp==3) fprintf (handle1, " ELTY %s TE4", set[i].name);
        if(set[i].etyp==4) fprintf (handle1, " ELTY %s HE20%c", set[i].name, attr);
        if(set[i].etyp==6) fprintf (handle1, " ELTY %s TE10", set[i].name);
        if(set[i].etyp==7) fprintf (handle1, " ELTY %s TR3%c", set[i].name, attr);
        if(set[i].etyp==8) fprintf (handle1, " ELTY %s TR6%c", set[i].name, attr);
        if(set[i].etyp==9) fprintf (handle1, " ELTY %s QU4", set[i].name);
        if(set[i].etyp==10) fprintf (handle1, " ELTY %s QU8", set[i].name);
        if(set[i].etyp==11) fprintf (handle1, " ELTY %s BE2", set[i].name);
        if(set[i].etyp==12) fprintf (handle1, " ELTY %s BE3", set[i].name);
        if(set[i].etyp>0)
	{ if(set[i].eparm!=(char *)NULL) fprintf(handle1, " %s\n", set[i].eparm); else fprintf(handle1, "\n"); }
      }
    }
  }
  else
  { 
    ...
    if(set[setNr].etyp==6) fprintf (handle1, " ELTY %s TE10", set[setNr].name);
    if(set[setNr].etyp==7) fprintf (handle1, " ELTY %s TR3%c", set[setNr].name, attr);
    if(set[setNr].etyp==8) fprintf (handle1, " ELTY %s TR6", set[setNr].name);
    if(set[setNr].etyp==9) fprintf (handle1, " ELTY %s QU4", set[setNr].name);
    if(set[setNr].etyp==10) fprintf (handle1, " ELTY %s QU8", set[setNr].name);
    if(set[setNr].etyp==11) fprintf (handle1, " ELTY %s BE2", set[setNr].name);
    if(set[setNr].etyp==12) fprintf (handle1, " ELTY %s BE3", set[setNr].name);
    if(set[setNr].etyp>0)
