...
{
  int  setNr, length, i, j, n=0, ipuf, elem2nr=0, comp, faces;
...
void sendPressure( char *setname, char *format, Summen *anz, Nodes *node, Elements *e_enqire, char *val1, char *val2, char *val3, char *val4 )
{
  ...
  if(val4) sprintf(&prognam[strlen(prognam)],"%s.bouf",val4);
        ...
        if(e_enqire[face[i].elem_nr].type>10)  // beam
	{
        ...
        else if(e_enqire[face[i].elem_nr].attr>3)  // plain strain,stress or axissym
        {
          /* only the surrounding faces are applicable */
                  /*
          if((face[i].nr==1)&&(orif==1)) fprintf( handle, "%d, PP, %lf\n", face[i].elem_nr, pressure[j]);
          else if((face[i].nr==1)&&(orif==0)) fprintf( handle, "%d, PN, %lf\n", face[i].elem_nr, pressure[j]);
          else
                  */
  ...
  if (compare( format, "abq", 3)== 3)
  {
    ...
    for (j=0; j<set[setNr].anz_f; j++ )
    {
     ...
     if(e_enqire[face[i].elem_nr].type>6)  // shell
      {
        if(e_enqire[face[i].elem_nr].type>10)  // beam
          fprintf( handle, "%d, F?, %lf, %e \n", face[i].elem_nr, temp[j], alpha[j]);
        else if(e_enqire[face[i].elem_nr].attr>3)  // plain strain,stress or axissym
        {
          //if(face[i].nr==0) fprintf( handle, "%d, FN, %lf, %e\n", face[i].elem_nr, temp[j], alpha[j]);
          //else if(face[i].nr==1) fprintf( handle, "%d, FP, %lf, %e\n", face[i].elem_nr, temp[j], alpha[j]);
          if((face[i].nr==1)&&(orif==1)) fprintf( handle, "%d, FP, %lf, %e\n", face[i].elem_nr, temp[j], alpha[j]);
          else if((face[i].nr==1)&&(orif==0)) fprintf( handle, "%d, FN, %lf, %e\n", face[i].elem_nr, temp[j], alpha[j]);
          else fprintf( handle, "%d, F%d, %lf, %e\n", face[i].elem_nr, face[i].nr-1, temp[j], alpha[j]);
        }
        else
        {
          //if(face[i].nr==0) fprintf( handle, "%d, FNEG, %lf, %e\n", face[i].elem_nr, temp[j], alpha[j]);
          //else if(face[i].nr==1) fprintf( handle, "%d, FPOS, %lf, %e\n", face[i].elem_nr, temp[j], alpha[j]);
          if((face[i].nr==1)&&(orif==1)) fprintf( handle, "%d, FP, %lf, %e\n", face[i].elem_nr, temp[j], alpha[j]);
          else if((face[i].nr==1)&&(orif==0)) fprintf( handle, "%d, FN, %lf, %e\n", face[i].elem_nr, temp[j], alpha[j]);
          else fprintf( handle, "%d, F%d, %lf, %e\n", face[i].elem_nr, face[i].nr+1, temp[j], alpha[j]);
        }
