...
{
  int i,j,k,n;
  int maxe=0, n1,n2, flag;
  double vl[3],*dist=NULL,max_length;
  double *clength=NULL, dist_length, dist_maxlength;
  Points *cg=NULL;
  ...
  /* determine the distance of the cg of the biggest tri to the nurbs,
     calc the ratio of this distance to the circumferential length of the tri
   */

  /* scan all tri and calc the cg and their clength */
  ...
  if( (clength = (double *)malloc( (e)*sizeof(double ))) == NULL )
  { printf(" ERROR: malloc failure in calcMeshDistance()\n\n"); return(0.); }
  ...
  for(i=0; i<e; i++)
  {
    ...
    for(n=0; n<3; n++)
    {
      ...
      {
        if(n1l[n1].n2[k]==n2) { clength[i]+=n1l[n1].l[k]; flag=0; break; }
      }
      if(flag==1)
      {
        ...
        clength[i]+=n1l[n2].l[n1l[n2].sum];
        n1l[n2].sum++;
      }
    }

    if(clength[i]>max_length) { max_length=clength[i]; maxe=i; }
  ...
  /* proj cg to nurbs */
  dist=projPntsToNurbs( nurbs, nr, e, cg);

  dist_maxlength=0.;
  for(i=0; i<e; i++)
  {
    dist_length=dabs(dist[i])/clength[i];
...
int triSplitter( Nurbs *nurbs, int nr, int *ptr_np,  Points **ptr_ppre, int *ptr_ne, int **ptr_tri, int **ptr_pntflag, int **edge_n2, int *pnt_indx, double **ptr_pntu, double **ptr_pntv)
{
  ...
  int np,ne, *tri;
  double p1[3]={0.,0.,0.},p2[3]={0.,0.,0.},p1p2[3],p1pm[3], pbuf[3], u=0.5;
  ...
  */
  ...
  for(loop=0; loop<MAX_MESH2D_LOOPS; loop++)
  {
    ...
    /* project pm to the nurbs and determine the distance between pm and the nurbs */
    //for(i=0; i<nmp; i++) printf("hallo %d pm:%f %f %f \n",i, pm2[i].px, pm2[i].py, pm2[i].pz);
    //if(nurbs[nr].nurbsType>2)
    if(nurbs[nr].nurbsType>10)
    {
      dist=projPntsToNurbs( nurbs, nr, nmp, pm);
      // activate cppfreearray(dist); at end of function
    }
    else
    {
    ...
    */
    ...
    for (k=0; k<ne; k++)
    {
      ...
      {
        nm=tribuf[k].nm[n];
        if((nm!=-1)&&(dist[nm]/(ln1n2[nm]*10.) > MESH2D_QUALITY_THRESHOLD)) tribuf[k].mod[n]=1;
      }
    }
  ...
  finishLoop:;
    ...
    //if(nurbs[nr].nurbsType<=2) { free(dist); dist=NULL; }
