...
/* factor for the geometric tolerance for merging */
...
//#define     GTOL_COS_A 0.9999     
#define     GTOL_COS_A 0.984     
/* Defines the default maximum distance between nodes during automatic line division calculation */
/* GTOL_NODE_DIST * gtol = target distance between nodes (maximum element-length, not exactly achieved) */
#define     GTOL_NODE_DIST 5.e2
#define     ELEM_LENGTH_RATIO 0.3      // defines the default minimum-element-length/maximum-element-length, used at curves

/* meshing parameters for the unstructured tr3 mesher (mesh2d) */
#define     NADAPT   8                 // for mesh2d()
#define     ALPHA    0.8               // mesh2d(): alfa, beta both 0.8 results in an unchanged average mesh density
#define     BETA     0.8
...
#define     MESH2D_QUALITY_THRESHOLD 0.01     // dist_cg_nurbs/circumlength_of_tri (curvature, 1st parameter)
#define     MESH2D_CURVATURE_FACTOR 20.      // sensitivity to surface curvature, 2nd parameter
...
// (1- dist*(MESH2D_CURVATURE_FACTOR) / tri_circumlength) >= MESH2D_QUALITY_THRESHOLD
...
/* default location of the help files, please insert the actual one */
  ...
  #else
    #define     HELPFILE        {"/yaprod/cae/application/CalculiX/doc/cgx_2.8/cgx.html",\
                             "/yaprod/cae/application/CalculiX/doc/ccx_2.8/ccx.html"}
  #endif
#elif defined AFLIB
...
#define     MAX_PARAM_PER_RECORD 20
#define     GL_FEEDBACK_BUFF_SIZE 300000  /* size of the feedback buffer for nurbs-rendering */
...
void calcSequence( DsSequence dsSequence, double anim_faktor, int halfperiode, int centerNode, Summen *anz, Nodes *n_anim, Nodes *node, Elements *e_enqire, Datasets *lcase, Scale *scale, char surfFlag );
...
int  shape_i( char *name, int type, int ip1, int ip2, int ip3, int ip4 );
...
void drawModelEdges( GLuint list_model_edges, int color, double width,
                     int numEdges, Nodes *node, Edges *edge );
void drawDispListEdges( GLuint list, int color, double width, char key );
...
void drawDispList( GLuint list, char key, Nodes *node );
...
void sendDflux( char *setname, char *format, Summen *anz, Nodes *node, Elements *e_enqire, char *val1, char *val2, char *val3 );
...
void drawElements_plot( int num, int *name, Nodes *node, Elements *e_enqire, int col, char type, int pickflag );
void drawElemNodes_plot( int num, int *name, Nodes *node, Elements *e_enqire, int col, char type );
void drawFaces_plot( int num, int *name, Nodes *node, Faces *face, int col, char type, int pickflag );
...
void areampc(int set1, int set2, char *format, char *type, char *value, char *corr, int offset, Nodes *nptr);
...
int createBlendedNurbs(int nr, Points **pntpntr, Lines *line, Lcmb *lcmb, Gsur *surf, Nurbs **nurbs );
...
double calcMeshDistance( Nurbs *nurbs, int nr, int np,  Points *ppre, int e, int *tri);
int triSplitter( Nurbs *nurbs, int nr, int *ptr_np,  Points **ptr_ppre, int *ptr_ne, int **ptr_tri, int **ptr_pntflag, int **edge_n2, int *pnt_indx, double **ptr_pntu, double **ptr_pntv);
