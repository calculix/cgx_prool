...
int abqrecord( char *string)
{
 ...
 getNextLine:;
  ...
  /* check for comments */
  do{ l=arecord( handlein, string); }while((string[0]=='*')&&(string[1]=='*'));
...
int getBoundaries(char *rec_str)
{
  ...
  do{
    ...
    for(i=0; i<80; i++) ival[i]=0; fval=0.;
...
{
  int i,j,n,args,setFlag=0,setNr=0, length;
...
int calcMaxMinValues(int l)
{
  ...
  {
    lcasex[l].max[i]=-MAX_INTEGER;
    lcasex[l].min[i]=MAX_INTEGER;
    for(j=0; j<anzx->n; j++)
    {
...
{
  int i,j,n; 
  int length, nd, foundLoads=0, nset, args, comp;
  double t;
  ...
  for(i=comp; i<lcasex[anzx->l].ncomps; i++)
  {
    ...
    if ( (lcasex[anzx->l].icname[i] = (char *)malloc( MAX_LINE_LENGTH * sizeof(char))) == NULL )
      printf("\n\n ERROR: malloc failed\n\n" );
    lcasex[anzx->l].max[i]=-MAX_INTEGER;
    lcasex[anzx->l].min[i]=MAX_INTEGER;
    ...
    lcasex[anzx->l].max[i]=-MAX_INTEGER;  lcasex[anzx->l].min[i]=MAX_INTEGER;
  }
  ...
  do{
    ...
    if (nd==0)
    {
      ...
      if(nset<0)
      {
        printf("ERROR: In *%s set:%s not defined\n", type, dat[0]);
        return(-1);
      }
      for(j=0; j<setx[nset].anz_n; j++)
      {
...
{
  int i,j; 
  int length, nd, dof, foundLoads=0,nset,args;
  double f;
  static int setNr[7]={0,0,0,0,0,0,0};
  static int local_step_nr=0, anzx_l;
    ...
    if(toupper(dat[1][0])=='P')
    {
      ...
      if(face>=0)
      {
        if (el==0)
        {
          ...
          if(eset<0)
          {
            printf("ERROR: In *DLOAD set:%s not defined\n", dat[0]);
            return(-1);
          }
          for(j=0; j<setx[eset].anz_e; j++)
          {
            ...
            if(i>-1)
    	    {
              setx[setNr].elf[i].e=setx[eset].elem[j];
              setx[setNr].elf[i].f=face;
...
int getRadiation(char *rec_str)
{
  ...
  do{
    ...
    if(face<0) printf(" ERROR in *RADIATE, face from string:%s unknown\n", buffer);
    ...
    if(face>=0)
    {
      if (el==0)
      {
          ...
          if(i>-1)
  	  {
          ...
          writeFaceVals(anzx_l, setx[eset].elem[j], face, 2, vals, 0);
        }
      }
      else
      {
...
int getFilm(char *rec_str)
{
  ...
  do{
    ...
    if(face<0) printf(" ERROR in *FILM, face from string:%s unknown\n", buffer);
...
int getSpcf(char *rec_str)
{
  ...
  else if(step_nr>local_step_nr)
  {
    ...
    strcpy ( lcasex[anzx_l+3].name, "DOF4");
    strcpy ( lcasex[anzx_l+4].name, "DOF5");
    strcpy ( lcasex[anzx_l+5].name, "DOF6");
    strcpy ( lcasex[anzx_l+6].name, "PRESS");
    strcpy ( lcasex[anzx_l+7].name, "TEMP ");
  }  
  do{
    length=abqrecord(rec_str);
    if(length==-1) return(-1);
    ...
    if(face<0) printf(" ERROR in *BOUNDARYF, face from string:%s unknown\n", buffer);
    for(i=0; i<12; i++)   vals[i]=0.;
    dof[0]=atoi(dat[2]);
    dof[1]=atoi(dat[3]);
    for(i=0; i<2; i++)
    {
      if(dof[i]==DOFP) dof[i]=7;
      if(dof[i]==DOFT) dof[i]=8;
    }
    ...
    if(face>=0)
    {
      if (el==0)
      {
        ...
        if(eset<0)
        {
          printf("ERROR: In *BOUNDARYF set:%s not defined\n", dat[0]);
          return(-1);
        }
        for(j=0; j<setx[eset].anz_e; j++)
        {
          ...
          if(i>-1)
  	  {
          ...
          for(i=dof[0]-1; i<=dof[1]-1; i++) writeFaceVals(anzx_l+i, setx[eset].elem[j], face, 1, &vals[i], 0);
        }
      }
      else
      {
        ...
        for(i=dof[0]-1; i<=dof[1]-1; i++) writeFaceVals(anzx_l+i, el, face, 1, &vals[i], 0);
      }
    }
      ...
      else if((compare(dat[i],"STRESS",6)==6) || (compare(dat[i],"PLASTICSTRAIN",13)==13) || (compare(dat[i],"PLASTIC STRAIN",14)==14))
      {
        ...
        for(j=0; j<lcasex[anzx->l].ncomps; j++)
        {
          ...
          if ( (lcasex[anzx->l].icname[j] = (char *)malloc( MAX_LINE_LENGTH * sizeof(char))) == NULL )
            printf("\n\n ERROR: malloc failed\n\n" );
          lcasex[anzx->l].max[j]=-MAX_INTEGER;
          lcasex[anzx->l].min[j]=MAX_INTEGER;
          ...
          lcasex[anzx->l].max[j]=-MAX_INTEGER;  lcasex[anzx->l].min[j]=MAX_INTEGER;          
        }
...
{
  int i,j, setNr, anzx_l=0, sector=0;
  char *string;
	...
	printf(" ncomps:%d\n",lcasex[anzx_l].ncomps); 
        ...
        for(i=0; i<lcasex[anzx_l].ncomps; i++)
        {
          ...
          if ( (lcasex[anzx_l].icname[i] = (char *)calloc( MAX_LINE_LENGTH, sizeof(char))) == NULL )
            printf("\n\n ERROR: malloc failed\n\n" );
          lcasex[anzx_l].max[i]=-MAX_INTEGER;
          lcasex[anzx_l].min[i]=MAX_INTEGER;
