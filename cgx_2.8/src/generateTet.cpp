...
int generateTetFromSet(int setNr, double teth )
{
  ...
  if ( (  readNG( "test.vol", anz_ng, &node_ng, &elem_ng, NULL)) == -1) 
  {
...
int generateTetFromBody(int nr, double teth )
{
  ...
  int i,j,k,n,s,sb;
  int setNr, snodSet, snodSet2, np, ne, ngtet[10], cgxtet[10], sumtri=0, sumtri3=0, sumtri6=0,n1,n2,nm;
  //double ngpnt[3];
  static int *cgxnode=NULL;
  static int *ngnode=NULL;
  char buffer[1000];
  ...
  for(sb=0; sb<body[nr].ns; sb++)
  {
    ...
    for (i = 0; i < surf[s].ne; i++)
    {
      ...
      else if (e_enqire[surf[s].elem[i]].type == 8) sumtri6++;
      else
      {
