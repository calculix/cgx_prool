  ...
  if (compare(type, "se0", 3) == 3 ) /* delete empty sets */
  {
    for (n=0; n<anz->sets; n++)
    {
      if((set[n].name != (char *)NULL )&&
         ...
         (!set[n].anz_nurs)&&
         (!set[n].anz_se)&&
	(!set[n].anz_sh)) { delSet(set[n].name); return; }
    }
    return;
  }
...
void pre_split( char *record)
{
  ...
  int p1_nr, p2_nr, ps_nr, l_nr[2], sum_psplt, orig_anz_l, orig_anz_s;
...
*/
 ...
 nextEdge:;
  ...
  if(set[setNr].anz_l)
  {
    orig_anz_l=set[setNr].anz_l;
    /* go over all lines and calculate the split-point */
    for (ll=0; ll<orig_anz_l; ll++)
    {
      l=set[setNr].line[ll];
  ...
  orig_anz_s=set[setNr].anz_s;
  for (ss=0; ss<orig_anz_s; ss++)
  {
    s=set[setNr].surf[ss];
    /* go over all tri */
    n=0;
...
*/  
  ...
  else if( compare( action, "_nor", 3) == 3)
  {
    ...
    {
      projSetToNurbs( nurbs, set[trgtNr].nurs[i], set, setNrbuf, pproj);
    }
    if(getSetNr(specialset->zap)>-1) delSet(specialset->zap);
  }
  else
...
int brecord( char *rec_str, char dat[MAX_PARAM_PER_RECORD][MAX_LINE_LENGTH])
{
  register int i;
...
int commandoInterpreter( char *type, char *string, int na, int nb, FILE *handle1, int addFlag, int *gtolFlag )
{
  ...
  */
  ...
  else if (compareStrings(type, "GTOL")>0)
  {
    ...
    else { gtol=calcGTOL(setall);  printf ("gtol set to:%e\n", gtol); }
  }
  ...
  else if (compareStrings(type, "NURS")>0)
  {
    ...
    if((length==2) && ( name[0] == '!' ))
    {
          ...
          if(k>-1)
	  {
            ...
            k=shape_i( nurbs[k].name, 4, k, 0, 0, 0);
            surf[nr].sh=k;
            repNurs(k);
            repSurf(nr,0);
	  }
...
int completeSet( char *setname, char *type )
{
  ...
  else if (toupper(type[0])=='D') /* down */
  {
    ...
    for (i=0; i<set[setNr].anz_sh; i++)
    {
      ...
      else if(shape[s].type==1) for (j=0; j<2; j++) seta( setNr, "p", shape[s].p[j] );
