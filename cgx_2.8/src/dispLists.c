...
void drawModelEdges( GLuint list_model_edges, int color, double width, int numEdges, Nodes *node, Edges *edge )
{
  int i;
...
void drawDispList( GLuint list, char key, Nodes *node )
{
  ...
  for (j=0; j<anzGeo->psets; j++ )
  {
    if (pset[j].type[0]==key)
    {
      if(key=='f') drawFaces_plot( set[pset[j].nr].anz_f, set[pset[j].nr].face, node, face, pset[j].col, pset[j].type[1],!PICK );
      if(key=='e') drawElements_plot( set[pset[j].nr].anz_e, set[pset[j].nr].elem, node, e_enqire, pset[j].col, pset[j].type[1],!PICK );
    }
  }
  glEndList();
}
