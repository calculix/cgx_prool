...
void help( void )
{
  ...
  printf ("   'prnt' ['usr']|['par' <RETURN|parameter>]|['amp' <RETURN|amplitude|*chars*>]|['mat' <RETURN|material|*chars*>]|['se'|'sq' <RETURN|set|*chars*>]|['n'|'e' <RETURN|set|*chars*> ['range']]|['n'|'e'|'f'|'p'|'l'|'s'|'b'|'v'|'S'|'L' <entity>]\n");
...
int surfToShape(int s)
{
  ...
  sbuf=shape_i( buffer, 0, pbuf[0], pbuf[1], pbuf[2], 0);
...
int cylToNurs(int s)
{
  ...
  //radius=shape[surf[s].sh].r[0]/ scale->w;
  ...
  p_lhq[6][1]=0.     ;
  p_lhq[7][1]=-radius;
  for(i=0; i<8; i++) p_lhq[i+8][1]=p_lhq[i][1];
  p_lhq[0][2]=0.     ;
  p_lhq[1][2]=-radius;
  p_lhq[2][2]=-radius;
  p_lhq[3][2]=-radius;
  ...
  p_lhq[6][2]= radius;
  p_lhq[7][2]= radius;
  for(i=0; i<8; i++) p_lhq[i+8][2]=p_lhq[i][2];
...
int surfToNurs(int s)
{
  ...
  if(shape[surf[s].sh].type==1) return(cylToNurs(s));
  if(shape[surf[s].sh].type>1) return(-1);
...
int repSurf(int nr, int renderFlag )
{
  ...
  if(( surf[nr].name != (char *)NULL )&&(surf[nr].sh>-1)&&(surf[nr].nc>0))
  {
    ...
    if(Stmp>-1)
    {
      ...
      sh_buf=surf[nr].sh;
      surf[nr].sh=shape_i( nurbs[Stmp].name, 4, Stmp, 0, 0, 0);
      if(surf[nr].pgn!=NULL)
      {
