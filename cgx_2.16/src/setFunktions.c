...
int pre_mesh( char *record )
{
  ...
  if(tetFlag)
  {
    ...
    generateTetFromSet(se, teth, set[se].eattr);
  ...
  else
  {
	...
	printf("\n Tet-mesh body:%s\n\n", body[c].name);
        if(body[c].eparm!=(char *)NULL) etmp=generateTetFromBody(c, (double)atof(body[c].eparm), body[c].eattr);
        else etmp=generateTetFromBody(c, (double)1.e6, body[c].eattr);
        adjustDrawNodes(1);
...
void setLightAndMaterial_rgb(int col, float alpha)
{
