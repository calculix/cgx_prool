...
void drawNodes_plot( int num, int *name, Nodes *node , int col, char type, int width)
{
  ...
  if(type=='t')
  {
	...
	{
          buffer[0]=0;
          if((ntext[j].text)&&(ntext[j].tFlag)&&(ntext[j].pFlag==1)) sprintf(buffer,"%s ",ntext[j].text);
          if((ntext[j].nFlag)&&(ntext[j].vFlag)&&(anz->l))
	  {
            switch(ntext[j].fFlag)
	    {
	    case 0:
               sprintf(&buffer[strlen(buffer)],"%d %6.2le", name[i], lcase[cur_lc].dat[cur_entity][name[i]]);
	      break;
	    case 1:
               sprintf(&buffer[strlen(buffer)],"%d %.2f", name[i], lcase[cur_lc].dat[cur_entity][name[i]]);
	      break;
	    case 2:
	      sprintf(&buffer[strlen(buffer)],"%d %d", name[i], (int)lcase[cur_lc].dat[cur_entity][name[i]]);
	      break;
	      printf("ERROR case:%d\n", ntext[j].fFlag);
	    }
	  }
          else if(ntext[j].nFlag) sprintf(&buffer[strlen(buffer)],"%d", name[i]);
          else if((ntext[j].vFlag)&&(anz->l))
	  {
            switch(ntext[j].fFlag)
	    {
	    case 0:
               sprintf(&buffer[strlen(buffer)],"%6.2le", lcase[cur_lc].dat[cur_entity][name[i]]);
	      break;
	    case 1:
               sprintf(&buffer[strlen(buffer)],"%.2f", lcase[cur_lc].dat[cur_entity][name[i]]);
	      break;
	    case 2:
	      sprintf(&buffer[strlen(buffer)],"%d", (int)lcase[cur_lc].dat[cur_entity][name[i]]);
	      break;
	      printf("ERROR case:%d\n", ntext[j].fFlag);
	    }
	  }
          if((ntext[j].text)&&(ntext[j].tFlag)&&(ntext[j].pFlag==0)) sprintf(&buffer[strlen(buffer)]," %s",ntext[j].text);
...
void drawElements_plot( int num, int *name, Nodes *node, double *colNr, Elements *e_enqire, int col, char type, int width, int pickflag )
{
  ...
  {
    setLightAndMaterial_rgb(col, alpha);
    glEnable( GL_LIGHTING );
...
void drawLines_plot( int num, int *name, Lines *line , Points *point, int col, char type, int width)
{
  ...
  if(anz_p)  drawPoints_plot( anz_p, pnt_indx, point, col, 0, width+3 );
}
