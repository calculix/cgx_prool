  ...
  for (i=0; i<22; i++)
  {
    ...
    if((i==1)&&(setNr==-2))
    {
      ...
      if(setNr>-1)
      {
        ...
        {
          e=set[setNr].face[i];
          if(elem_define( anz->enext++, face[e].type, face[e].nod, 1, 0 ) <0 ) errMsg("WARNING: element %d could not be created\n", anz->enext--);
	}
        return;
      }
    }
...
int pre_value( char *record)
{
  ...
  char parameter[MAX_STACK_PARAMS][MAX_LINE_LENGTH], param[MAX_LINE_LENGTH];
  char splitkey[2]={0,0};
  ...
  {
    printf(" Waiting for user input. Please type into the terminal:\n");
    i=0;
    do
    {
      string[i]=getchar(); i++;
