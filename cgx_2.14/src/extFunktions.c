...
void help( void )
{
  ...
  printf ("   'dist' <set> <set>|<shpe> []|['tra' <x> <y> <z> <offset> [<tol>]]|['rad' [<p1> <p2> <offset>]|['x'|'y'|'z' <offset>] [<tol>]]|['rot' [<p1> <p2> <offset>]|['x'|'y'|'z' <offset> [<tol>]]]|['nor' <offset> [<tol>]]\n"); 
  printf ("   'div'  []|[<default>]|[<line> <div>]|[<set> ['auto' <node-dist> <angle> <elem-ratio>]|['mult'|'div' <factor-div> [<factor-bias>]]]\n");
  printf ("   'ds' [[<1.Dataset-Nr>|'l'|<negative-ds> [<2.Dataset-Nr>|'l'|<negative-ds> [<3.Dataset-Nr>|'l'|<negative-ds>]]] ['a[h]' [<entity-nr>]]|['e[h]' <entity-nr> (up to 4 for vector-plots)]|['o' <offset> [<entity-nr>]]|['p' <power>] [<entity-nr>]]|['s' <factor> [<entity-nr>]]]|['g' <name> [[<ncomps>|<0>] <value> <text> <type> <step> <analysisName>]]|['e' <name> <comp> <type> <row> <column>]|['r' <key> [<parm1>] [..<parm5>]\n");
  printf ("   'elem' <nr|!> [set]|[<firstNode>-<lastNode> 'be2'|'be3'|'tr3'|'tr6'|'qu4'|'qu8'|'he8'|'he20']\n");
  printf ("   'elty' [] | [<set> 'be2'|'be2f'|'be2d'|'be3'|'be3f'|'tr3'|'tr3u'|'tr3e'|'tr3s'|'tr3c'|'tr6'|'tr6u'|'tr6e'|'tr6s'|'tr6c'|'qu4'|'qu4e'|'qu4s'|'qu4c'|'qu4r'|'qu4er'|'qu4sr'|'qu4cr'|'qu8'|'qu8e'|'qu8s'|'qu8c'|'qu8r'|'qu8er'|'qu8sr'|'qu8cr'|'he8'|'he8f'|'he8i'|'he8r'|'he20'|'he20r'|'te4'|'te4f'|'te10'|'te10m'|'te10t'|'pe6'|'pe6f'|'pe6i'|'pe15'|'pe15r' [<parameter>]]\n");
  printf ("   'else'\n");
  printf ("   'endif'\n");
  printf ("   'endwhile'\n");
  ...
  printf ("   'node' <nr|!> [<x> <y> <z>]|['v' <value> [<value> .. ]] \n");
  printf ("   'norm' <set> \n");
  ...
  printf ("   'ori' <set>\n");
  printf ("   'plot' ['n'|'e'|'f'|'p'|'l'|'s'|'b'|'S'|'L'|'sh']&['a'|'d'|'p'|'q'|'t'|'v'] <set> ['w'|'k'|'n'|'r'|'g'|'b'|'y'|'m'|'t'] [<width>]\n");
  printf ("   'plus' ['n'|'e'|'f'|'p'|'l'|'s'|'b'|'S'|'L'|'sh']&['a'|'d'|'p'|'q'|'t'|'v'] <set> ['w'|'k'|'n'|'r'|'g'|'b'|'y'|'m'|'t'] [<width>]\n");
  ...
  printf ("   'txt' <set> ['n']|[['v'|'e'|'f'|'i']&['n']] [<x> <y>]\n");
  printf ("   'ucut' \n");
  printf ("   'ulin' <string>\n");
  ...
  printf ("   'view' 'fill'|'line'|'point' <value>|'edge' ['off'|<value>]|'elem' ['off']|'surf'|'volu'|'front'|'back'|'vec' ['off']|'disp' ['off']|'bg' ['w'|'k']|'sh' ['off']|'ill' ['off']\n");
  printf ("   'volu' <set>\n");
...
void repShape(int setNr )
{
  ...
  for (j=0; j<set[setNr].anz_sh; j++)
  {
    ...
    {
      printf(" WARNING: shape:%s type:%d, surface rendering not implemented\n", shape[nr].name, shape[nr].type );
    }
  }
}
...
int repSurf(int nr, int renderFlag )
{
  ...
  if(( surf[nr].name != (char *)NULL )&&(surf[nr].sh>-1)&&(surf[nr].nc>0))
  {
        ...
        if(i>0)
	{
      ...
      if(renderFlag)
      {
        /* the triangulation using GLu func works only with active graphics, should be substituted by mesh2d */
        if(!inpformat) { surf[nr].npgn=0; goto jumpGLuRendering; }
...
void adjustDrawNodes(int flag)
{
  ...
  if(flag)
  {
    ...
    for (lc=0; lc<anz->l; lc++)
    {
	  ...
	  for(j=0; j<n; j++ ) lcase[lc].dat[i][anz->orignmax+j]=0.;
        }
      }
    }
  }
...
int write2stack(int n, char parameter[MAX_STACK_PARAMS][MAX_LINE_LENGTH])
{
  int i;
