...
int pre_while(char *type, char *string, int *na, int *nb, FILE *handle1, int *addFlag, int *gtolFlag, int wFlag)
{
  ...
  else if(compare( type,"WHILE",2)==2 )
  {
    ...
    //printf("hallo WHILE: %s\n",string);
    //printf(" args:%s %s\n",arg1,arg2);
    //printf(" nwhiles:%d stackptr:%d flag:%d\n", nwhiles, stack_ptr, flag);
    //if(stack_ptr>-1) printf(" stack[%d].flag:%d\n",stack_ptr, stack[stack_ptr].flag);
    if(flag==1) return(1);  // error or strings are not equal


    if(wFlag==-3) // restore the next command
    {
      ...
      if(flag==3) stack[stack_ptr].flag=3;    // restore and execute (all commands are stored and repeated)
  ...
  {
    //printf("hallo ENDWHILE stackptr:%d stack[stack_ptr].flag:%d\n",stack_ptr, stack[stack_ptr].flag);
...
int separateMeshes( char *setName)
{
  int i,e,k=2;
...
*/
 ...
 nextEdge:;
        ...
        if((m==3)&&(c==2)) /* (cut along edge through tet => 2new tets) */
	{
	  ...
	  {
            elem_define( anz->enext++, 3, e_enqire[e].nod, 1, 0 );
	  }
          else
	  {
            elem_define( anz->enext++, 6, e_enqire[e].nod, 1, 0 );
	  }
        ...
        else if((m==2)&&(c==2)) /* 2 node copied (cut along an edge) */
	{
          ...
          if(e_enqire[e].type==5)
	  {
            ...
            elem_define( anz->enext++, 5, nel, 1, 0 );
          ...
          {
            elem_define( anz->enext++, 2, nel, 1, 0 );
            seta(set_ntop, "n",nel[1]);
            seta(set_ntop, "n",nel[2]);
            seta(set_ntop, "n",nel[4]);
            seta(set_ntop, "n",nel[5]);
...
#define PROJ_LOOPS 16
  ...
  char parameter[MAX_STACK_PARAMS][MAX_LINE_LENGTH];
  ...
  /* if the type is proj and the direction nor then project directly to the shapes (offset in this case not regarded) */
...
int prnt(char *record)
{
  ...
  else if ((compare( typ, "info", 2) == 2) ||(compare( typ, "INFO", 2) == 2))
  {
    ...
    printf ("%s %s n:%d e:%d f:%d ed:%d t:%d se:%d mat:%d amp:%d ds:%d nmax:%d nmin:%d emax:%d emin:%d nnext:%d enext:%d\n", datin, anz->model, anz->n, anz->e, anz->f, anz->g, anz->t, anz->sets, anz->mats, anz->amps, anz->l, anz->nmax, nmin, anz->emax, emin, anz->nnext, anz->enext);
    ...
    sprintf(parameter[11],"%d",anz->nmax);
    ...
    sprintf(parameter[17],"%d",anzGeo->p);
    sprintf(parameter[18],"%d",anzGeo->l);
    sprintf(parameter[19],"%d",anzGeo->c);
    sprintf(parameter[20],"%d",anzGeo->s);
    sprintf(parameter[21],"%d",anzGeo->b);
    sprintf(parameter[22],"%d",anzGeo->sh);
    sprintf(parameter[23],"%d",anzGeo->nurl);
    sprintf(parameter[24],"%d",anzGeo->nurs);
    sprintf(parameter[25],"%d",anzGeo->psets);
    write2stack( 26, parameter);
  }
      ...
      for (i=0; i<anz->sets; i++)
      {
        if( set[i].name != (char *)NULL )
        {
          if ((!set[i].type)&&(set[i].name[0]!=':'))
          {
            ...
            sprintf(parameter[2],"%d",set[i].flag);
...
int zap(char *record)
{
  ...
  if (set[setNr].anz_sh>0)
  {
    ...
    for ( i=0; i<set[setNr].anz_s; i++)
    {
      shape[set[setNr].shp[i]].ns=iremove(&shape[set[setNr].shp[i]].s, shape[set[setNr].shp[i]].ns, set[setNr].surf[i]);
    } 
...
void placeTxt(int setNr, int j, int k, int l, char dat[MAX_PARAM_IN_BRECORD][MAX_LINE_LENGTH])
{
  int   i, n, nr;
  GLint    viewport[4];
  ...
  for(i=0; i<set[setNr].anz_n; i++)
  {
    ...
    if(j>-1)
    {
      if(dat[j][0]=='n') ntext[nr].nFlag=0;
      else if(dat[j][1]=='n') ntext[nr].nFlag=0;
      if(dat[j][0]=='v') ntext[nr].vFlag=0;
      else if(dat[j][0]=='e') ntext[nr].fFlag=0;
      else if(dat[j][0]=='f') ntext[nr].fFlag=1;
      else if(dat[j][0]=='i') ntext[nr].fFlag=2;
    }
    if((k)||(l))
    {
      ...
      ntext[nr].tx= wx+(double)(k);
      ntext[nr].ty= (viewport[3]-wy)-(double)(l);
    }
  }
}
...
int commandoInterpreter( char *type, char *string, int na, int nb, FILE *handle1, int addFlag, int *gtolFlag )
{
  ...
  double    lfbuf, pscal, x,y,dx,dy, wx[2], wy[2];
  char      **substring=NULL;
  int       foundSubString;
  ...
  */
  ...
  else if (compareStrings(type, "MAXR")>0)
  {
    ...
    sscanf( string, "%*s %lf %c %c",&pscal,&scale->format,&scale->lock );
    scale->smaxr=1;
  ...
  else if (compareStrings(type, "NODE")>0)
  {
    ...
    {
      //if((cur_lc)&&(cur_lc<anz->l)&&(cur_lc)&&(cur_entity<lcase[cur_lc].ncomps))
      if(anz->l)
      {
        /* check if the data of the specified lcase (Dataset) are already available */
        if (!lcase[cur_lc].loaded)
  ...
  else if (compareStrings(type, "THRS")>0)
  {
    ...
    if(comm[0]=='t')
    {
      ...
      ibuf=separateMeshes(specialset->thrs);
      ...
      for(i=1; i<=ibuf; i++)
      {
        sprintf(buffer,"%s%d %sN%d rec _ _ _ 0. %s", specialset->cf, i, specialset->cf, i, format );
        enquireEntities(buffer);
        sprintf(buffer,"%sN%d", specialset->cf, i);
  ...
  {
    args=sscanf(&string[na],"%s %s %s %s", name, dat[0], dat[1], dat[2]);
    setNr=getSetNr(name);
    if (setNr<0)
    {
      printf (" txt: set:%s does not exist\n", name);
    ...
    if(args==1) { k=0; l=0; j=-1; }
    else if(checkIfNumber(dat[0])) { x=atof(dat[0]); y=atof(dat[1]); j=-1; k=x*width_w1; l=y*height_w1; }
    else if(args==2) { k=0; l=0; j=0; }
    else { x=atof(dat[1]); y=atof(dat[2]); j=0; k=x*width_w1; l=y*height_w1; }
    placeTxt(setNr, j, k, l, (char (*)[MAX_LINE_LENGTH]) &dat[0][0]);
  }
